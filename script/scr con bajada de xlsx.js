...
function decode(text){
 ...
 // console.log("obj" ,obj); 
  ...
  //console.log("p:",p);
	var tt = tiempo.slice(0,480);
	var vv =  valor.slice(0,480);
	var data2 = {
	  labels: tt, //["0s", "10s", "20s", "30s", "40s", "50s", "60s"], //at
	  datasets: [{
		label: "Tempertura °C",
		data: vv, // [0, 59, 75, 20, 20, 55, 40], // Temp
		borderColor: 'orange',
...
document.getElementById("Descargar").disabled = false; // habilita el boton para desargar la informacion
}
...
function generate(type) {
        return getWorkbook()
            .then(function (workbook) {
				d = new Date();
				fecha = d.getDate().toString()+'/'+(d.getMonth()+1).toString()+'/'+ d.getFullYear().toString();
				...
				for(i=0; i<largo; i++){
					ind = i + 6;
					workbook.sheet(0).cell("B"+ind).value( tiempo[i] );
					workbook.sheet(0).cell("C"+ind).value( valor[i] );
				}
...
function generateBlob() {
	d = new Date();
		nombre ="GreenBeats_"+ d.getDate().toString() +(d.getMonth()+1).toString() + d.getFullYear().toString()+".xlsx";
        return generate()
            .then(function (blob) {
